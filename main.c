...
char *cifradoAES256(char buf[100])
{   
    aes256_context ctx; 
    aes256_init(&ctx, key);
...
char *descifradoAES256(char buf[100])
{
    aes256_context ctx; 
    aes256_init(&ctx, key);
...
void menuConfiguracion(sqlite3 *db)
{
    ...
    char sql[140] = "INSERT INTO usuarios(hash) VALUES('";
    strcat(sql, enc);
    strcat(sql, "')");
    sqlite3_exec(db, sql, callback, NULL, NULL);
    //printf("%s", sql);
...
void usuarioNull()
{
    ...
    char sql[140] = "SELECT * FROM usuarios";
    sqlite3_exec(db, sql, callback, NULL, NULL);
    if(status == 0)
    {   
...
void validarLogin()
{
    ...
    cifradoAES256(pass);
    char *enc = b64_encode(pass, strlen(pass));
    char sql[140] = "SELECT hash FROM usuarios Where hash = '";
    strcat(sql, enc);
    strcat(sql, "')");
    printf("\n");
    sqlite3_exec(db, sql, callback, NULL, NULL);
